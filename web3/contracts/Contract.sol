...
contract FlashPay {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "FlashPay: caller is not the owner");
        _;
    }
    ...
    modifier onlyVerifiedToken(address _token) {
        require(
            ...
            "FlashPay: token is not verified"
        );
        _;
    }
    ...
    function removeVerifiedToken(address _token) public onlyOwner {
        require(
            verifiedTokens[_token] == true,
            "FlashPay: token is not in the list"
        );
    ...
    function transfer(
        ...
        require(senderBalance >= amount, "FlashPay: insufficient balance");

        bool success = token.transferFrom(msg.sender, to, amount);
        require(success, "FlashPay: transfer failed");
